...
public class BookingTools {
    ...
    public String listPackagesByCategory(
            @ToolArg(description = "Lista os pacotes de viagem disponiveis para uma determinada categoria (ex: ADVENTURE, TREASURES).") Category category
    ) {
        List<Booking> packages = bookingService.findByCategory(category);
        if (packages.isEmpty()) {
